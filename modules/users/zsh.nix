...
in
  outputs.lib.mkModule config "zsh" {
    options' = {
      notify = outputs.lib.mkEnableOption "Notify on long commands";
    };
        ...
        zsh = {
          ...
          antidote = {
            ...
            plugins =
              ...
              ++ outputs.lib.optionals cfg.notify [
          ...
          initContent = let
            zshConfigEarlyInit = outputs.lib.mkOrder 500 (''
              ...
              ''
              + outputs.lib.optionalString cfg.notify ''
            ...
            zshConfig = outputs.lib.mkOrder 1000 ''
              ...
              ${
                if config.modules.fastfetch.enable
                then outputs.lib.getExe pkgs.fastfetch
                else ""
              }
            '';
          in
