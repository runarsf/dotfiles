{ outputs, config, inputs, pkgs, name, ... }:
...
# TODO Optionally use system-wide sops module, look in nginx and stylix config
{
...
} // outputs.lib.mkModule' config "sops" (with outputs.lib; {
  ageKeys = mkOption {
    default = [ ];
  ...
  privateKeyNames = mkOption {
    default = let
      ageKeyNames = config.modules.sops.ageKeys
        |> map (key: outputs.lib.splitString "/" key)
        |> map (key: outputs.lib.last key);
      in ageKeyNames ++ config.modules.sops.privateKeys;
    readOnly = true;
  };
}) {
