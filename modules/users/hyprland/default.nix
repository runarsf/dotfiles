...
// outputs.lib.mkDesktopModule config "Hyprland" {
  ...
  nixos.programs.xwayland.enable = true;
  nixos.programs.hyprland.xwayland.enable = true;
  ...
  wayland.windowManager.hyprland =
    ...
    in
    {
      ...
      plugins = with inputs.hyprland-plugins.packages.${system}; [ borders-plus-plus ];
      settings = {
        ...
        exec = [ "${pkgs.swww}/bin/swww kill; ${pkgs.swww}/bin/swww query || ${pkgs.swww}/bin/swww init" ];
        general = {
          gaps_in = 5;
          gaps_out = 20;
          border_size = 1;
