...
__conda_setup="$("${HOME}/.local/share/miniforge3/bin/conda" 'shell.zsh' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "${HOME}/.local/share/miniforge3/etc/profile.d/conda.sh" ]; then
        . "${HOME}/.local/share/miniforge3/etc/profile.d/conda.sh"
    else
...
if [ -f "${HOME}/.local/share/miniforge3/etc/profile.d/mamba.sh" ]; then
    . "${HOME}/.local/share/miniforge3/etc/profile.d/mamba.sh"
fi
# <<< conda initialize <<<
